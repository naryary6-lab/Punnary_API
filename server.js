...
// 2. Link ABA KHQR Code (សូមផ្លាស់ប្តូរទៅជា Direct Image Link)
const ABA_QR_IMAGE_URL = 'https://drive.google.com/file/d/1vmVJGZqj-DaQvuQTRrMmVoU_prKfC0g0/view?usp=drivesdk';
...
// HTML / Frontend Mini App ពេញលេញ ២០ មុខទំនិញ
const miniAppHTML = `
<!DOCTYPE html>
<html lang="km">
<head>
...
<body>
  ...
  <div class="container">
    ...
    <div class="cart-section">
      ...
      <div class="qr-container">
        ...
        <img src="https://via.placeholder.com/190x190?text=KHQR+Code" alt="KHQR Code" class="qr-image">
        <div class="thank-you-text">អរគុណច្រើនសម្រាប់ការគាំទ្រ!</div>
      </div>

      <button class="btn-telegram" onclick="sendToTelegram()">
        ✈️ ផ្ញើការកុម្ម៉ង់ចូល Telegram
      </button>
  ...
  <script>
    ...
    function sendToTelegram() {
      let selectedProducts = products.filter(p => p.qty > 0);
      let totalItems = selectedProducts.reduce((sum, p) => sum + p.qty, 0);
      ...
      let freeItems = Math.floor(totalItems / 5);
      let subTotal = selectedProducts.reduce((sum, p) => sum + (p.price * p.qty), 0);
      let actualShipping = totalItems >= 5 ? 0 : standardShipping;
      let finalTotal = subTotal + actualShipping;
      ...
      if (tg && tg.initData) {
        ...
        if (freeItems > 0) orderText += `🎁 ថែមជូនឥតគិតថ្លៃ៖ ${freeItems} កំប៉ុង\n`;
        orderText += `🚚 ថ្លៃដឹកជញ្ជូន៖ ${actualShipping === 0 ? 'FREE (ឥតគិតថ្លៃ)' : '7,000 ៛'}\n`;
        orderText += `-------------------\n💰 ទឹកប្រាក់សរុបត្រូវទូទាត់៖ ${finalTotal.toLocaleString()} ៛`;

        window.open(`https://t.me/+85567678956?text=${encodeURIComponent(orderText)}`, '_blank');
      }
    }
...
// AI Chatbot & Telegram WebApp Data Handler
bot.on('message', async (msg) => {
    const chatId = msg.chat.id;

    if (msg.text && !msg.web_app_data) {
        if (msg.text === '/start') {
            bot.sendMessage(chatId, "ស្វាគមន៍មកកាន់ ម៉ូម័រ MOMORE Snack Store! 🍌\nសូមចុចប៊ូតុងខាងក្រោមដើម្បីបើកហាងទំនិញ ឬសួរសំណួរផ្សេងៗ៖", {
                reply_markup: {
                    keyboard: [[{ text: "🛒 បើកហាងទំនិញ (Mini App)", web_app: { url: "https://pennary-api-1.onrender.com" } }]],
        ...
        try {
            ...
            const prompt = `អ្នកគឺជាអ្នកប្រឹក្សានិងលក់ទំនិញរបស់ហាង "MOMORE" (ម៉ូម័រ)។ ហាងយើងខ្ញុំលក់ដំណាប់ចេកខ្មែរ សាច់ទន់ ស្អិត រសជាតិដើម ព្រមទាំងសម្រន់ជាច្រើនមុខ។ ឆ្លើយតបខ្លីៗ រួសរាយ និងសមរម្យ៖ "${msg.text}"`;
            const result = await model.generateContent(prompt);
            const response = await result.response;
            bot.sendMessage(chatId, response.text());
        } catch (error) {
            bot.sendMessage(chatId, "សូមទោសចាស! MOMORE Bot កំពុងមមាញឹក បងអាចចុចមើល Mini App ដើម្បីបញ្ជាទិញបានចាស!");
        }
    }

    // ទទួលទិន្នន័យពី Mini App
    if (msg.web_app_data) {
        try {
            const data = JSON.parse(msg.web_app_data.data);
            let summary = "🧾 **វិក្កយបត្របញ្ជាទិញពី MOMORE!**\n-------------------\n";
            
            data.cart.forEach((item, index) => {
                summary += `📦 ${index + 1}. ${item.name} (${item.qty} កំប៉ុង) = ${(item.price * item.qty).toLocaleString()} ៛\n`;
            });

            if (data.freeItems > 0) {
                summary += `🎁 **ថែមជូនឥតគិតថ្លៃ៖** ${data.freeItems} កំប៉ុង\n`;
            }

            summary += `🚚 **ថ្លៃដឹកជញ្ជូន៖** ${data.shipping === 0 ? 'FREE (ឥតគិតថ្លៃ)' : '7,000 ៛'}\n`;
            summary += `-------------------\n💰 **ទឹកប្រាក់សរុបត្រូវទូទាត់៖ ${data.finalTotal.toLocaleString()} ៛**\n\nសូមស្កែន ABA KHQR ខាងក្រោមដើម្បីបង់ប្រាក់៖`;

            await bot.sendMessage(chatId, summary, { parse_mode: 'Markdown' });
            await bot.sendPhoto(chatId, ABA_QR_IMAGE_URL, { caption: "សូមផ្ញើរូបភាពវិក្កយបត្រ (Receipt) បន្ទាប់ពីវេរប្រាក់រួច! អរគុណច្រើនសម្រាប់ការគាំទ្រ! 🍌" });
        } catch (e) {
            bot.sendMessage(chatId, "មានបញ្ហាក្នុងការប្រមូលទិន្នន័យបញ្ជាទិញ សូមសាកល្បងម្តងទៀតចាស!");
        }
    }
});
